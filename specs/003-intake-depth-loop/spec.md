...
- **FR-017**: The compliance metamodel (canonical requirements, controls, evidence specifications, provision mappings, requirement→tier and requirement→control mappings) MUST be **seeded** as governed reference/seed data (separate from demo data). [NEEDS CLARIFICATION: metamodel seed breadth — a curated *starter* set across the perimeter frameworks (EU AI Act, NAIC Model Bulletin, NY DFS, Colorado SB21-169, GDPR), or a broader/exhaustive catalog? Starter is the assumed default.]
- **FR-018**: [NEEDS CLARIFICATION: registry-asset prerequisite for P2 — does this feature include a **minimal registry-asset primitive** (create an executable + version and advance its lifecycle, enough to link and promote) plus a thin linking/promote UI, with the full Studio/Registry authoring experience deferred to a later feature? That minimal primitive is the assumed default, since asset linking + the promotion gate cannot be demonstrated without assets to link.]
...
- **FR-021**: The assessment questionnaire and the risk-tier scoring MUST map to / derive from canonical-requirement tier criteria, so that completing the assessment both computes the tier **and** identifies the applicable requirement tiers — rather than a standalone bespoke catalog. [NEEDS CLARIFICATION: reconciliation approach for 002's existing bespoke assessment — (a) refactor the questionnaire + scoring to be fully metamodel-driven now; (b) keep the 002 questionnaire but add a mapping layer (each answer → canonical-requirement tier criteria) that drives resolution; or (c) consume the 002 assessment output (tier/domains/frameworks) as the resolution input now and metamodel-drive the questionnaire in a later slice. Option (b) is the assumed default — preserves the shipped UX while making resolution metamodel-true.]
...
- **002's assessment is currently bespoke** (the `FIELDS` catalog + `compute_tier`); reconciling it to the metamodel (FR-021) is in scope, with the assumed default being a mapping layer (each answer → canonical-requirement tier criteria) rather than a full rewrite, so the shipped UX is preserved while resolution becomes metamodel-true.
