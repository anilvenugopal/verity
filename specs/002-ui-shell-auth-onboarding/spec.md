...
**M4 scope guardrail**: M4 surfaces *only* intake backend that has already shipped in `001` — intake CRUD (slice 1), the two shipped assessment tabs (AI Decision Impact + Data) with the computed tier/materiality readout (slice 3), and submit → tier-quorum approval (slice 4). It explicitly does **not** include the Security & Access tab, mitigations, the Risk & Obligations summary tab (FR-AS-004–010), obligation-resolution display (FR-IN-014), or change-proposal flows (FR-IN-013) — that backend is not built; those screens land in feature `003` alongside their backend. M4 reuses the M3 **tab-gated sign-off gate** (extracted from the application workspace) with `kind=intake`.

> **Scope exception (2026-06-09, owner decision):** to give the intake lifecycle the *same* requester/app-team affordances as application onboarding (so the UX is familiar), M4 **adds** a small, well-bounded slice of intake backend — pre-approval **edit** (`PUT /intakes/{id}`), **withdraw** (`POST /intakes/{id}/withdraw`), **delete** (`DELETE /intakes/{id}`), and `requested_changes`-closes parity in the intake sign-off. This is the only deliberate deviation from "frontend-only over shipped backend"; it mirrors the application lifecycle one-to-one (FR-033–FR-035) and is fully tested. Everything else in the guardrail still holds.
...
- Q: Should the intake lifecycle match application onboarding's approve / cancel (withdraw) / delete affordances, even though that backend hadn't shipped? → A: **Yes — build the missing intake backend.** Add pre-approval edit / withdraw / delete (mirroring the application lifecycle) so the requester and app-team experience is identical and familiar (FR-033–FR-035); this is the recorded scope exception above.
- Q: (Supersedes the 2026-06-05 reject-only answer) Now that intake has a withdraw route and `requested_changes` closes the intake request, what decisions does the shared sign-off gate offer for `kind=intake`? → A: **Approve / Request changes / Reject** — the same three as onboarding. Both "Request changes" and "Reject" close the request (status → rejected); the intake stays at `in_review` (revisable) for **Edit & re-submit** — full FR-019 parity. (The earlier reject-only restriction no longer applies.)

### Session 2026-06-05

- Q: For M4's intake sign-off view — the shipped intake-approval backend has no withdraw/return route. Reject-only, or also "Return for revision"? → A: **Reject-only** (approve/reject). The M3 "Return for revision" button is omitted for `kind=intake`; a rejected sign-off resolves the request as rejected. (Stays inside M4's shipped-only scope.) **[SUPERSEDED 2026-06-09 — see above: the gate now offers Approve / Request changes / Reject, matching onboarding.]**
...
1. **Given** an authenticated user, **when** they navigate to `/applications`, **then** the registry renders with a search field, a table showing name, owner, review status (`ReviewBadge`), and line of business, and an "Onboard application" CTA visible only to roles with `onboard_application` permission.
2. **Given** the registry search field, **when** a user types, **then** results filter to matching application names in real time.
3. **Given** a user clicks an application row, **then** the **application workspace** (`/applications/{id}`) opens as a read view — identity band, Compliance + Ownership tabs, Risk Profile + Governance & Approval rail — with edit/lifecycle affordances shown only to onboard-capable users for a still-`pending` app.
4. **Given** a user with `onboard_application` permission, **when** they click "Onboard application", **then** the workspace **create mode** opens — an Identity card above Compliance/Ownership tabs with Prev/Next and per-field required markers.
5. **Given** the onboard form, **when** the user completes all required fields and submits, **then** `POST /applications` (+ `/submit`) is called and the user lands in the new application's workspace.
6. **Given** incomplete required fields, **when** the user clicks Submit, **then** an error summary lists the missing fields, the offending tabs are marked, and the first gap is focused (the button is not silently disabled).
...
8. **Given** a pending application awaiting approval, **when** an approver opens its workspace, **then** the Governance & Approval rail shows the sign-off gate, and Approve / Request changes / Reject are disabled until the approver has reviewed **every tab** (tab-gate).
9. **Given** the gate enabled, **when** the approver clicks "Approve", **then** `POST /approvals/{approval_request_id}/signoff` is called with `decision_code: "approved"`; once the quorum is satisfied the application transitions to `active` and the rail reflects it.
10. **Given** the gate, **when** the approver clicks "Reject" or "Request changes", **then** `POST /approvals/{approval_request_id}/signoff` is called (`rejected` / `requested_changes`); per 001 FR-IN-015a the request **closes** (→ rejected) and the application returns to an editable Draft, remediated via **Edit & re-submit** (which supersedes the prior pending approval).
11. **Given** an application, **when** the user navigates to `/applications/{id}`, **then** the **workspace** renders the identity band + tabs (Compliance, Ownership & team; **Use cases** in M4) + Risk Profile / Governance rail + collapsed History; the Use cases tab shows an empty-state CTA when no intakes exist.
...
3. **Given** an open `kind=intake` approval, **when** an approver opens the intake detail, **then** its Governance & Approval panel reuses the M3 **tab-gated sign-off gate** (`intake.usecase-review`) sourced from `GET /approvals/{approval_request_id}`, showing the composed intake and the quorum progress (which roles have signed), with **approve/reject** actions only — no "Request changes" button.
...
5. **Given** the sign-off gate with the tab-gate satisfied, **when** an approver holding a required role signs "approve", **then** `POST /approvals/{approval_request_id}/signoff` is called with `decision_code: "approved"`; the quorum progress updates.
...
- What happens when a `viewer`-only user opens the registry? → "Onboard application" CTA is absent; rows open the application workspace as a read view (no edit/lifecycle affordances).
...
*(FR-014–020 reconciled 2026-06-09 to the shipped surface: the application page is a single `ApplicationWorkspace` — not a modal/4-tab detail — and the approval is tab-gated in its governance rail; see 001 FR-IN-015a for the remediation/lifecycle backend.)*

- **FR-014**: The applications registry MUST source from `GET /applications`, support real-time client-side search, and show the "Onboard application" CTA only to users with the **`onboard_application`** permission (the action the backend route gates on).
- **FR-015**: Clicking an application row MUST open the **application workspace page** (`/applications/{id}`) — a read view; edit/lifecycle affordances appear only to onboard-capable users and only for a still-`pending` application.
- **FR-016**: The onboard form is the workspace **create mode**: an **Identity card** above tabbed segments (Compliance, Ownership & team) with **Prev/Next**, and an industry-standard **required-field UX** — per-field `*` markers, a live "what's left" count, inline errors after blur/attempt, and an on-submit summary that focuses the first gap (no silently-disabled submit). Final submit calls `POST /applications` (+ `POST /applications/{id}/submit` to open the approval, landing in the workspace).
...
- **FR-018**: The approval/sign-off lives in the workspace **Governance & Approval rail**; the decision actions MUST be disabled until the approver has reviewed **every tab** (a **tab-gate**, the workspace analogue of the original scroll-gate).
- **FR-019**: The approval decision MUST be recorded via the shared approval primitive `POST /approvals/{approval_request_id}/signoff` (the `approval_request_id` comes from `POST /applications/{application_id}/submit` or `GET /applications/{application_id}/approval`). The rail offers **Approve** (`approved`), **Request changes** (`requested_changes`), and **Reject** (`rejected`) — the real `reference.approval_decision` vocabulary. Per 001 FR-IN-015a, a `rejected` **or** `requested_changes` sign-off **closes** the request (status → rejected; no deadlock); the application returns to an editable **Draft** and is remediated via **Edit & re-submit** (re-submit supersedes any prior pending approval). A full quorum of `approved` → request `approved` + application `active`. Application lifecycle actions (Edit & re-submit, Cancel request, Delete) live in a separate **"Application actions" footer**, NOT the governance rail. There is no `/applications/{id}/approve` route; `withdraw` (requester cancel) and `DELETE` (app-team, pending-only) exist per 001 FR-IN-015a.
- **FR-020**: The application page is the **`ApplicationWorkspace`** (`/applications/{id}`), sourced from `GET /applications/{id}`: an **identity band** (name · TLA · review status · owner · LOB · onboarded), tabs (**Compliance**, **Ownership & team**; a **Use cases** tab is added in M4 — FR-023), an always-visible right rail (**Risk Profile** + **Governance & Approval**), and a collapsed **History of changes**. The derived review status (Draft / In review / Rejected / Changes requested) is shown via `ReviewBadge`.
...
- **FR-026** (revised — A+B+C+D redesign): The assessment MUST be **one sectioned form** (not peer tabs — a single shared validation gate split across tabs hid required fields), with a section nav showing per-section complete/incomplete/error state and validation that scrolls the first error into view. Sections: **Decision context · Data inventory · Human oversight · Risks · Fairness** (data-model §10). **Data inventory**, **human-oversight controls**, **risks**, and **fairness metrics** MUST be **multi-entry** (add/edit/remove items), per EU AI Act Art 10/14, NIST MAP, OECD, and the reference-implementation pattern (Microsoft RAIIA, ICO DPIA). Every consequential field MUST carry layered guidance — inline helper text, a `(?)` tooltip, and a "Learn more" affordance explaining each option and its consequence, grounded in the framework language. The unbuilt obligation/mitigation-resolution surfaces remain in feature `003`.
- **FR-027**: The assessment saves the **full snapshot** in one `PUT /intakes/{intake_id}/assessment` (the backend captures the whole assessment as one SCD-2 revision — no partial PUT), writing a revision and recomputing the tier. The intake-level `data_classification` and PII presence are **derived from the data inventory** (max classification / strongest PII across items) and validated against the application ceiling. The computed risk tier, NAIC materiality, and derived classification MUST render in a read-only summary after save. Re-opening MUST reload captured answers via `GET /intakes/{intake_id}/assessment`, with prior revisions from `GET /intakes/{intake_id}/assessment/revisions`.
...
- **FR-029**: The intake sign-off MUST be presented on the **intake detail page's Governance & Approval panel** (`intake.usecase-review`), reusing the **shared, tab-gated sign-off gate** extracted from the application workspace (the `.appr` quorum rows + decision actions) with `kind=intake`, sourced from `GET /approvals/{approval_request_id}` (resolved via `GET /intakes/{intake_id}` → its open approval), showing the composed intake and quorum progress; sign-off MUST call `POST /approvals/{approval_request_id}/signoff`. The gate MUST offer the **same three decisions as onboarding — Approve (`approved`) / Request changes (`requested_changes`) / Reject (`rejected`)**; per the 2026-06-09 parity decision, a `requested_changes` **or** `rejected` sign-off closes the request (status → rejected) and the intake stays at `in_review` (revisable) for **Edit & re-submit**. (The standalone `ApprovalView` from the original M3 plan was folded into the workspace rail; the intake reuse target is that shared gate, not a separate page.)
...
- **FR-031**: Every M4 screen MUST have a defined empty state and MUST disable all write affordances for a **locked** intake — one whose status is not a pre-decision authoring state. Revisable = `{proposed, in_review, impact_assessment}`; locked = `{approved, rejected, retired, in_build, live}`. (A quorum rejection leaves the intake at `in_review` — revisable — so a rejected *sign-off* does not lock the intake; only an explicit `rejected`/`retired` triage does.)
...
- **FR-033**: The intake detail page MUST present the intake's pre-approval lifecycle actions in an **"Intake actions" footer** (NOT the Governance & Approval rail), mirroring the application workspace footer: **Edit & re-submit**, **Cancel request**, and **Delete**. The footer and its actions MUST be shown only for a **revisable** intake (FR-031) and hidden/disabled for a locked one.
- **FR-034**: **Edit & re-submit** MUST route to an intake edit mode that prefills title/description and saves via `PUT /intakes/{intake_id}` (gated `edit_intake`); **Cancel request** (the requester withdrawing their own submission) MUST call `POST /intakes/{intake_id}/withdraw` (gated `edit_intake`; shown only when a pending approval exists) and return the intake to a revisable draft (status unchanged). These mirror the application **Edit & re-submit** / **Cancel request** affordances exactly.
- **FR-035**: **Delete** MUST call `DELETE /intakes/{intake_id}` and be shown only to users with the `delete_intake` permission (the app team — `business_owner`/`ai_governance`/`security`), only for a revisable intake; on success it MUST navigate back to the owning application's Use cases tab. This mirrors the application workspace **Delete** affordance.
...
- M4 integrates against intake backend that **already exists** in the governance service (`hub` modules `intake/`, `assessment/`, `intake_approval/`, `approval/`); M4 is **mostly** frontend plus the same read-wiring pattern as M1–M3. The **one** backend addition is the pre-approval lifecycle-parity slice (edit / withdraw / delete + `requested_changes`-closes), per the 2026-06-09 scope exception (FR-033–FR-035). The unbuilt assessment tabs and change-proposal flows remain deferred to feature `003`.
